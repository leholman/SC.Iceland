#############################################
####=== SeaChange WP4 Analysis B05-2006==####
####==== Luke E. Holman====01.02.2024====####
#############################################
...
pool <- gsub("(.*R)1_001.fastq.gz", "\\1", filename)
looppoolName <- paste0("pool",gsub(".*-2006-(\\d+).*", "\\1", filename))
 
temp <- system2("cutadapt",args=paste0("-e 1 --pair-adapters --trimmed-only -j 8 -g ^file:P.Forwards.fasta -G ^file:P.Reverses.fasta -o S.demultiplexed/",looppoolName,"-{name}.S.R1.fastq.gz -p S.demultiplexed/",looppoolName,"-{name}.S.R2.fastq.gz ",pool,"1_001.fastq.gz ",pool,"2_001.fastq.gz"), stdout=T, stderr=T)
cat(file="log.txt", temp, append=T, sep="\n")

temp <- system2("cutadapt",args=paste0("-e 1 --pair-adapters --trimmed-only -j 8 -g ^file:P.Reverses.fasta -G ^file:P.Forwards.fasta -o A.demultiplexed/",looppoolName,"-{name}.A.R1.fastq.gz -p A.demultiplexed/",looppoolName,"-{name}.A.R2.fastq.gz ",pool,"1_001.fastq.gz ",pool,"2_001.fastq.gz"), stdout=T, stderr=T)
cat(file="log.txt", temp, append=T, sep="\n")
...
write.csv(t(EUK.TotalTable),"../6.mappings/OTUtabs/EUK.B05-2006.raw.csv")
...
write.table(as.data.frame(t(EUK.TotalTable.named)),"../6.mappings/OTUtabs/EUK.B05-2006raw.names.csv",sep=",")


##Lets try assigning to PR2

PR2assign <- assignTaxonomy( unlist(read.fasta("../5.OTUs/EUK.DADA2.ASVs.fasta",as.string = T)),refFasta = "../taxonomy/pr2_version_5.0.0_SSU_dada2.fasta.gz",tryRC = TRUE,multithread = TRUE,taxLevels = c("Domain","Supergroup","Division","Subdivision","Class","Order","Family","Genus","Species"),outputBootstraps = TRUE)
...
plotQualityProfile(fnFsSense[c(155:160)])
dev.off()
#...and reverses
pdf("../7.DADA2/S_RIZ_reverse.pdf")
plotQualityProfile(fnRsSense[c(155:160)])
dev.off()
...
write.csv(t(RIZ.TotalTable),"../6.mappings/OTUtabs/RIZ.B05-2006.raw.csv")
...
write.table(as.data.frame(t(RIZ.TotalTable.named)),"../6.mappings/OTUtabs/RIZ.B05-2006.raw.names.csv",sep=",")
...
plotQualityProfile(fnFsSense[c(911:912)])
dev.off()
#...and reverses
...
# Name the derep-class objects by the sample names
names(derepFs) <- sapply(strsplit(basename(list.files("../7.DADA2/filtered", pattern="MAM.*_S_F_filt.fastq.gz", full.names = TRUE)), "_"), `[`, 1)
names(derepRs) <- sapply(strsplit(basename(list.files("../7.DADA2/filtered", pattern="MAM.*_S_R_filt.fastq.gz", full.names = TRUE)), "_"), `[`, 1)
...
# Name the derep-class objects by the sample names
names(derepFs) <- sapply(strsplit(basename(list.files("../7.DADA2/filtered", pattern="MAM.*_A_F_filt.fastq.gz", full.names = TRUE)), "_"), `[`, 1)
names(derepRs) <- sapply(strsplit(basename(list.files("../7.DADA2/filtered", pattern="MAM.*_A_R_filt.fastq.gz", full.names = TRUE)), "_"), `[`, 1)
...
write.csv(t(MAM.TotalTable),"../6.mappings/OTUtabs/MAM.B05-2006.raw.csv")


#Output OTUs for taxonomy 
ASVs <- DNAStringSet(getSequences(MAM.TotalTable))
names(ASVs) <- paste0("ASV_",1:length(ASVs))
writeXStringSet(ASVs,"../5.OTUs/MAM.DADA2.ASVs.fasta", append=FALSE,
                compress=FALSE, compression_level=NA, format="fasta")
...
write.table(as.data.frame(t(MAM.TotalTable.named)),"../6.mappings/OTUtabs/MAM.B05-2006.raw.names.csv",sep=",")
...
euk.tax <- ParseTaxonomy(pctThreshold = 99,
                         covpct = 95,
                         blastoutput = "taxonomy/raw/EUK.B05.dada2.raw.taxonomy.txt",
                         lineages = "taxonomy/ncbi_lineages_2024-03-04.csv.gz")

mam.tax <- ParseTaxonomy(pctThreshold = 99,
                         covpct = 95,
                         blastoutput = "taxonomy/raw/MAM.B05.dada2.raw.taxonomy.txt",
                         lineages = "taxonomy/ncbi_lineages_2024-03-04.csv.gz")


riz.tax <- ParseTaxonomy(pctThreshold = 99,
                         covpct = 95,
                         blastoutput = "taxonomy/raw/RIZ.B05.dada2.raw.taxonomy.txt",
                         lineages = "taxonomy/ncbi_lineages_2024-03-04.csv.gz")
...
ApplyLulu(seqs="5.OTUs/EUK.DADA2.ASVs.fasta",
         table="6.mappings/OTUtabs/EUK.B05-2006raw.names.csv",
         output="8.LULU/EUK.B05.lulu.csv",
